...
@export var _steering_speed_deg := 135.0
...
func update(delta: float, axis: float) -> void:
	...
	var rate := _steering_speed if not is_zero_approx(axis) else (_return_speed * (3 * PI))
	_steering_angle = move_toward(_steering_angle, target, rate * delta)
	_steering_angle = sign(axis) * abs(_steering_angle)
